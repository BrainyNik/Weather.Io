...
const fetchWeatherData = async (location, latLon, units) => {
  ...
  if (latLon) {
    ...
    requests.push(
      axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${location}&units=${units}&appid=${API_KEY}`),
      axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${location}&units=${units}&appid=${API_KEY}`)
    );
  }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!latLon && currentWeatherResponse.data) {
          ...
          const airQualityData = await axios.get(`https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${API_KEY}`);
