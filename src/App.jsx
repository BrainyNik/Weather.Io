...
import { Cloud, Loader2Icon, UmbrellaOffIcon } from 'lucide-react'
function App() {

  const [searchQuery, setSearchQuery] = useState("");
  const [coord, setCoord] = useState(() => localStorage.getItem("coord") ? JSON.parse(localStorage.getItem("coord")) : null);

  const [unit, setUnit] = useState(() => localStorage.getItem("unit") ? JSON.parse(localStorage.getItem("unit")) : "metric");
  ...
  return (
    <>
      <div className="bg-background md:h-screen w-screen md:overflow-hidden  md:grid md:grid-cols-3">
        <Navbar searchQuery={searchQuery} setSearchQuery={setSearchQuery} setCoord={setCoord} unit={unit} setUnit={setUnit} />
        <div className="mt-[80px] md:p-0 sm:p-4 ">
          ...
          />
          <DaysForecast forecastData={forecast} unit={unit} />
        </div>
        <div className="mt-[80px] md:col-span-2 overflow-y-auto custom-scrollbar">
          ...
          <TodayAt forecastData={forecast} timezone={currentWeather?.timezone} unit={unit} />
        </div>
      </div>
    </>
  )
